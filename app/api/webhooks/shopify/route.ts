...
async function selectAgent(productId: string): Promise<string | null> {
  ...
  if (product.distributionType === "specific" && product.agents.length > 0) {
    eligibleIds = product.agents
      .filter(pa => !pa.agent.suspended && pa.agent.isOnline && pa.agent.lastSeenAt && pa.agent.lastSeenAt >= threshold)
      .map(pa => pa.agent.id);
  } else {
  ...
  const counts = await Promise.all(
    eligibleIds.map(async id => ({
      id,
      count: await prisma.order.count({ where: { agentId: id, status: { isFinal: false } } }),
    }))
  );
...
// Same method as old app: uses SHOPIFY_WEBHOOK_SECRET env variable.
// Shopify → order/create → HMAC verified → order saved → agent assigned.
export async function POST(request: Request) {
  try {
    const rawBody    = await request.text();
    const hmacHeader = request.headers.get("x-shopify-hmac-sha256") ?? "";

    // ── Verify HMAC using env variable (like old app) ────────────────────
    // Priority 1: SHOPIFY_WEBHOOK_SECRET (env var — same as old app)
    // Priority 2: DB store secret (fallback for Settings-configured stores)
    const shopDomain = request.headers.get("x-shopify-shop-domain") ?? "";
    let webhookSecret = process.env.SHOPIFY_WEBHOOK_SECRET ?? "";

    if (!webhookSecret) {
      // Fallback: look up store in DB
      const store = await prisma.shopifyStore.findUnique({ where: { domain: shopDomain } });
      if (!store || !store.isActive) {
        return NextResponse.json({ error: "Store not configured. Set SHOPIFY_WEBHOOK_SECRET in env or add store in Settings." }, { status: 401 });
      }
    ...
    // ── Get or auto-create default status ────────────────────────────────
    const status = await ensureStatuses();
    if (!status) return NextResponse.json({ error: "No status available" }, { status: 500 });

    // ── Create order ──────────────────────────────────────────────────────
    const newOrder = await prisma.order.create({
      data: { customer, phone, city, price, quantity, revenue: price * quantity, statusId: status.id, productId: product.id },
    });

    // ── Assign to best available agent ────────────────────────────────────
    const agentId = await selectAgent(product.id);
    if (agentId) {
      await prisma.order.update({ where: { id: newOrder.id }, data: { agentId } });
    }

    console.log(`✅ Webhook: order ${newOrder.id} created for ${customer} | agent: ${agentId ?? "none"}`);
